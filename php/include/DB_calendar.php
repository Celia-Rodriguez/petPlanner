...
class DB{
    ...
    public static function checkUser($userName,$password){
        $sql = "SELECT user FROM usuario WHERE user = :userName  AND password = :passwaord;";
        $params =[
            ':userName:'=> $userName,
            ':password' => $password
        ];
...
/****** Mostrar TODOS los Recordatorios ******/
    public static function showAllReminders($id){
        $sql="SELECT pet_reminder.*, type_of_reminder.tipo, type_of_reminder.nombre_tipo FROM user_pet 
        INNER JOIN pet_reminder ON user_pet.id_pet = pet_reminder.id_pet 
        INNER JOIN type_of_reminder ON pet_reminder.id_tipo = type_of_reminder.id WHERE user_pet.id_user = :id;";
        $params= [':id' => $id];
